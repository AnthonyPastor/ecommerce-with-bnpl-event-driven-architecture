...
Subscribes to seven Kafka topics with one handler via `KafkaConsumerService.subscribe()` (from `@bnpl/kafka-client`), then dispatches by `envelope.eventType` to one use case each:
...
Most of these reuse the same `saveWithOutbox` transactional-outbox pattern as `order-service` (see that service's CLAUDE.md for the pattern itself) whenever they publish a new domain event, as noted per bullet above; hold/resume and a rescheduled retry are pure internal state changes with no outbox write.
